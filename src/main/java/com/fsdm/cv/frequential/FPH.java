...
public class FPH {
    ...
    public static File applyHighPassFilter(File imageFile, double cutoffFrequency, int filterOrder) {
        try {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    H_filter[y][x] = 1 - 1.0 / (1.0 + Math.pow(cutoffFrequency / D[y][x], 2 * filterOrder));
                }
            }
    ...
    public static void main(String[] args) {
        ...
        double cutoffFrequency = 30.0; // Example cutoff frequency
        int filterOrder = 2; // Example filter order
        File outputFile = applyHighPassFilter(inputFile, cutoffFrequency, filterOrder);
        if (outputFile != null) {
            System.out.println("Filtered image saved to " + outputFile.getPath());
        } else {
            System.out.println("Failed to filter image.");
        }
    }
}